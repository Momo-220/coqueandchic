  ...
  try {
    ...
    container.innerHTML = `
      ...
      <div class="product-detail-info">
        ...
        <a id="waLink" href="#" class="btn btn-outline" style="width: 100%; border-radius: 8px; font-weight: 600; padding: 1rem; text-decoration: none; display: flex; align-items: center; justify-content: center; gap: 0.5rem; box-sizing: border-box; height: 50px; font-size: 0.95rem; cursor: pointer;" target="_blank">
    ...
    document.getElementById('qtyPlus').onclick = () => {
      ...
      if (qty < maxStock) { qty++; document.getElementById('qtyVal').textContent = qty; }
    };
    // Mettre à jour le lien WhatsApp avec le numéro depuis les settings
    api.fetchSettings().then(settings => {
      const ownerPhone = (settings.ownerPhone || '+225 07 68 61 33 28').replace(/\D/g, '');
      const waLink = document.getElementById('waLink');
      if (waLink) waLink.href = `https://wa.me/${ownerPhone}?text=${encodeURIComponent('Bonjour, je suis intéressé par : ' + product.name)}`;
    });
    ...
    document.getElementById('favBtn').onclick = () => {
      ...
      if (btn) {
        btn.style.background = isFavNow ? 'var(--rose-primary)' : 'var(--rose-blush)';
        btn.style.color = isFavNow ? 'white' : 'var(--rose-deep)';
      }
      if (svg) svg.setAttribute('fill', isFavNow ? 'currentColor' : 'none');
      updateBadge();
      toast(isFavNow ? '❤️ Ajouté aux favoris' : 'Retiré des favoris');
    };
    }
  } catch (err) {
